...
func Plugin(ctx context.Context) *plugin.Plugin {
	p := &plugin.Plugin{
		...
		TableMap: map[string]*plugin.Table{
			...
			// "consul_agent_member":     tableConsulAgentMember(ctx),
			// "consul_deployment":       tableConsulDeployment(ctx),
			// "consul_job":              tableConsulJob(ctx),
			// "consul_namespace":        tableConsulNamespace(ctx),
			// "consul_node":             tableConsulNode(ctx),
			// "consul_plugin":           tableConsulPlugin(ctx),
